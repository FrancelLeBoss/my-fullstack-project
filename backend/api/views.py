...
>>>>>>> 9e69158ad30a0df5dfe39919eaa373b38a4e5ed6
                else:
...
def best_rated(request):
    ...
    # Récupérer les produits avec leurs évaluations
    ratings = Rating.objects.select_related("product")
    ...
    for rating in ratings:
        product = rating.product
        if product:
            avg_rating = sum(r.stars for r in product.ratings.all()) / product.ratings.count()
    ...
    # Prendre les 10 meilleurs produits
    top_products = [prod for prod, rating in product_ratings[:10]]
...
def save_comment(request):
    ...
    if missing:
        return Response(
            {"error": "Missing required fields.", "missing": missing, "received": received},
            status=HTTP_400_BAD_REQUEST,
        )
